...
const SearchResults = () => {
  ...
  //Contexts
  const { searchResults, isSearching, searchSource, query } = useSearchContext();
  ...
  return (
    <View className="flex-1 mt-[88px]">
      ...
      {isSearching ? (
        <View className="px-4">
          {Array.from({ length: 6 }).map((_, i) => <SearchResultSkeleton key={`skel-${i}`} />)}
        </View>
        ...
        <View className="flex-1 items-center justify-center gap-4 px-8">
          ...
          <Text className="text-white/40 font-nunito-600 text-lg text-center">
